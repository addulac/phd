...
* Generate the class membership distributions $\mat{F}_{N \times \infty}$:
   \begin{align}
...
We say that \ifm, in $\me$, satisfies the local preferential attachment effect iff for any indexing, for any node $i, \, 1\leq i \leq N$ such that $f_{ik}=1$, and for any step $p, \, 1\leq p < N$, $P(d_{i,k}^{(N)} \geq n+1 | d_{i,k}^{(p)}=n,\me)$ increases with $n$ ($1\leq n < p$). If $P(d_{i,k}^{(N)} \geq n+1 | d_{i,k}^{(p)}=n,\me)$ is independent of $n$, the model is said to be neutral wrt to the local preferential attachment effect.
\end{definition}
...
We say that \imb, in $\me$, satisfies the local preferential attachment effect iff for any indexing, for any node $i, \, 1\leq i \leq N$ such that $f_{ik}=1$, for any step $p, \, 1\leq p < N$, and for all $\epsilon$ compatible with the domain of definition of $d_{i,k}$ and $x$, $P(d_{i,k}^{(N)} \geq x+\epsilon | d_{i,k}^{(p)} \geq x,\me)$ increases with $x$. If $P(d_{i,k}^{(N)} \geq x+\epsilon | d_{i,k}^{(p)} \geq x,\me)$ is independent of $x$, the model is said to be neutral wrt to the local preferential attachment effect.
\end{definition}
...
Let us assume that elements of $\mat{\hat{F}}$ follow a power law distribution, \textit{i.e.} that for all $i$ and $k$, $pdf(\hat{f}_{ik}) \propto x^{- \alpha_{ik}}$ with $\alpha_{ik} > 1$ and where $pdf$ denotes the probability density function. Then \imb\ satisfies the local preferential attachment effect in $\me$.
\end{proposition}
...
\vspace{0.1cm}
The power law distribution assumed here for $\hat{f}_{ik}$ directly translates the positive reinforcement effect of the Dirichlet Process [@HDP] at the basis of \imb, that corresponds to a burstiness phenomenon. The definitions that we have considered for the (local and global) preferential attachment effects are indeed extensions of the definitions for burst iness proposed in [@clinchant2010information]. Power law distributions have been often used to model bursty phenomena ([@barabasi_burst][@clauset2009power] to cite but a few), and our assumption on $\hat{f}_{ik}$ reflects this usage.

<!--It is easy to see, using the same development as the one we have used, that power distributions are bursty in the sense of [@clinchant2010information].-->
...
* The p-value is defined as the fraction of the resulting statistics $KS_s, s \in \{1,...,S\}$ obtained on the samples larger than the value $KS_{obs}$ computed on the network distribution.
