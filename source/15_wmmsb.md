...
In order to capture this information and to alleviate the bias that often consist of thresholding the weighted networks to binary ones, weighted versions of the Stochastic Block Model have been proposed [@karrer2011stochastic;@mariadassou2010uncovering;@aicher2014learning;@peixoto2018nonparametric]. Those models however suffer from the same drawback as standard stochastic block models, namely the fact that a node can belong to only one class, which is not realistic for many networks. Mixed-membership block models were specifically designed to overcome this limitation and we propose here a new mixed-membership block model adapted to weighted networks. One important aspect in designing a generative model for networks is to develop a scalable inference method so that the model can be applied on large networks. We rely in this study on collapsed variational inference coupled with stochastic variational inference to do so.
