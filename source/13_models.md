...
\begin{table}[h]
    ...
    \begin{tabular}{c||l|c|c|c|c|c|}
        ...
        \multirow{5}{*}{\shortstack[1]{Latent\\class}} &
           SB      \cite{goldenberg2010survey} & Bernoulli & Multinomial           & no & ER \\
         & IRM     \cite{IRM}                  & Bernoulli & DP                    & no & SB \\
         & IHRM    \cite{IHRM}                 & Bernoulli & DP                    & no & IRM \\
         & MMSB    \cite{MMSB}                 & Bernoulli & Multinomial-Dirichlet & yes & SB \\
         & IMMSB   \cite{iMMSB} \cite{diMMSB}  & Bernoulli & HDP                   & yes & MMSB, IRM \\
        \hline
        \multirow{4}{*}{\shortstack[1]{Latent\\feature}} &
           LFL    \cite{menon2010log}          & Bernoulli & -                     & no & softmax \\
         & ILRM   \cite{ILFRM}                 & Bernoulli & IBP                   & yes & IRM \\
         & BPM    \cite{BMF}                   & Bernoulli & IBP                   & yes & IRM \\
         & IMRM   \cite{IMRM}                  & Bernoulli & IBP                   & yes & ILFM \\
    \end{tabular}}
...
This results is known as the de Finetti' theorem and constitute justification for the existence of a latent variable model under the exchangeability assumption.
...
This powerful theorem gives again a justification for the use of latent variables and most important the form of theirs priors (and a model in entirely determined by the choice of $F$) for exchangeable graph. Although, it is not straitfoward to proove that the representation for mixed-membership model given in Eq. \ref{eq:mmm} generate exchangeable graph, it appears that it is a special case of theorem \ref{th:aldous_hoover}, which has been shown in [@aldous1981representations] and [@kallenberg2006probabilistic]. It derives from the fact the edges probabilities are conditionally independent and that the nodes are associated to i.i.d random variables. In particular, the exchangeability of IRM, IMMSB and ILFM models are also illustrated in [@orbanz2015bayesian].

An notable corrolary of the theorem \ref{th:aldous_hoover} is that exchangeable graph are either dense (i.e the number of edges growth quadratically with $N$) or empty since their expected number of edges is independent of $N$. The study of representation for graph in the sparse regime are an active and growing field of research [@veitch2015class][@caron2017sparse][@le2015sparse][@bollobas2011sparse][@borgs2014p].
